...
class Weibo:
    cookie = {"Cookie": "SCF=ArVBgbY7rer3wuIwrYZxO5Ly8kcrKlB5X6eiCnvAamdAqPGWvsI1M21kgdP6j4DJ46D91ygBtRdXuuSCP6cRriw.; MLOGIN=1; _T_WM=2c42bd40ae091f5d5dc096206a9f9bf9; XSRF-TOKEN=cd188e; SUB=_2A25xYgC7DeThGeNN6lsT8SzFzz6IHXVSrKDzrDV6PUJbkdBeLUzBkW1NSdoP03N-J1YNis2uTVUZyEnOvoXR7os9; SUHB=0id7aK9iN12cOu; SSOLoginState=1550217451; M_WEIBOCN_PARAMS=luicode%3D20000174; WEIBOCN_FROM=1110106030"}  # 将your cookie替换成自己的cookie
    ...
    def write_list_txt(self):
        try:
            ...
            result=result + txt_title

            for i in range(1, self.weibo_num2 + 1):
                text = (str(i) + "\t"+
                        self.weibo_content[i - 1] + "\t"+
                       self.weibo_place[i - 1] + "\t"+
                        self.publish_time[i - 1] + "\t"+
                        str(self.up_num[i - 1]) +"\t"+
                        str(self.retweet_num[i - 1]) +"\t"+
    ...
    def write_txt(self):
        try:
            ...
            if not os.path.isdir(file_dir):
                os.mkdir(file_dir)
            file_path = file_dir + os.sep + "%d" % self.user_id + "_list.txt"
            f = open(file_path, "wb")
            f.write(result.encode(sys.stdout.encoding))
            f.close()
            print(u"微博写入文件完毕，保存路径:")
    ...
    def write_content_txt(self):
        try:
            ...
            if not os.path.isdir(file_dir):
                os.mkdir(file_dir)
            file_path = file_dir + os.sep + "%d" % self.user_id + "content.txt"
            f = open(file_path, "wb")
...
def main():
    try:
        ...
        user_id = 3911558393  # 可以改成任意合法的用户id（爬虫的微博id除外）
